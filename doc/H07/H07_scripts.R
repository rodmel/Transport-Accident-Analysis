...
sub <- with(NCDB,  NCDB[ ,c("C_YEAR", "C_RALN", "C_CONF")] )

## summary(sub)


# Exclude all those unknown or NA Road Surfaces
sub[ sub$C_RALN %in% c("Q","U","X") , "C_RALN" ] <- NA

sub$C_RALN  <- factor(sub$C_RALN
                      , levels=c("1","2","3","4","5","6")
                      , labels=c( "Straight and Level"
                                  ,"Straight With Gradient"
                                  ,"Curved and Level"
                                  ,"Curved With Gradient"
                                  ,"Top of Hill/Gradient"
                                  ,"Bottom of Hill/Gradient"
                      )
                )
...
colnames(D) <- c("Year","Road_Alignment","Collision_Type","Collision_Category")
